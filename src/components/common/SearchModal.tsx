...
const CATEGORY_TABS: Array<{ id: string; label: string }> = [
  ...
  { id: 'utilities', label: 'Tools & Utilities' },
];
...
export function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center pt-12 sm:pt-20 px-3 sm:px-4 bg-slate-950/70 backdrop-blur-md animate-fade-in"
      onClick={onClose}
    >
      <div
        className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden flex flex-col max-h-[85vh] animate-scale-up"
      ...
      >
        ...
        <div className="p-4 border-b border-slate-100 flex items-center gap-3">
          ...
          <input
            ref={inputRef}
            type="text"
            placeholder="Tool suchen (z. B. Word, Excel, Bild, Komprimieren, OCR, ZIP)..."
            value={query}
            onChange={(e) => {
              setQuery(e.target.value);
              setSelectedIndex(0);
            }}
            className="flex-1 bg-transparent text-slate-900 placeholder:text-slate-400 text-base sm:text-lg focus:outline-none"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
          ...
          <button
            onClick={onClose}
            className="hidden sm:flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-200 bg-slate-50 text-slate-500 hover:text-slate-800 text-xs font-mono"
          >
            <span>ESC</span>
          </button>
        </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 uppercase tracking-wider px-2 mb-2">
                    ...
                    <button
                      ...
                      className="text-2xs text-slate-400 hover:text-rose-600 lowercase tracking-normal"
                    >
                      Löschen
                    </button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {recentTools.map((tool) => (
                      <Link
                        ...
                        href={`/de/${tool.slug}`}
                        onClick={onClose}
                        className="flex items-center gap-2.5 p-2.5 rounded-xl border border-slate-200 bg-white hover:border-sky-300 hover:bg-sky-50/50 transition-all group"
                      >
                        <div
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {popularTools.map((tool) => (
                    <Link
                      ...
                      href={`/de/${tool.slug}`}
                      onClick={onClose}
                      className="flex items-center justify-between p-2.5 rounded-xl border border-slate-200 bg-white hover:border-sky-300 hover:bg-sky-50/50 transition-all group"
                    >
        ...
        <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-2xs text-slate-500">
