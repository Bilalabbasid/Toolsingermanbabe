...
export function PdfRepairOptimizeEngine({ 
  ...
  const processSinglePdf = async (
    ...
    if (mode === 'repair') {
      ...
      onProgress?.(30, 'Dokument an PDF/A-Archivierungsservice übermitteln...');
      const formData = new FormData();
      formData.append('file', file);
      formData.append('type', 'pdf_pdfa');
      ...
      if (!res.ok) {
        ...
        throw new Error(errJson.error || 'Serverfehler bei der PDF/A-Konvertierung.');
      }

      const jobData = await res.json();
      const jobId = jobData.jobId;

      onProgress?.(50, 'Ghostscript ISO 19005 Konvertierung läuft...');
      let attempts = 0;
      ...
      while (attempts < 60) {
        ...
        if (statusData.status === 'failed') {
          throw new Error(statusData.error || 'PDF/A-Konvertierung fehlgeschlagen.');
        }
      }

      if (!completedJob) throw new Error('Zeitüberschreitung bei der PDF/A-Konvertierung.');

      const downloadRes = await fetch(`/api/v1/jobs/${jobId}/download`);
      if (!downloadRes.ok) throw new Error('Download des PDF/A-Dokuments fehlgeschlagen.');

      const blob = await downloadRes.blob();
      const outName = `${file.name.replace(/\.[^/.]+$/, '')}_pdfa.pdf`;
      onProgress?.(100, 'Fertig');
      return {
        blob,
        fileName: outName,
        audit: 'Authentische ISO 19005-1 (PDF/A-1b) Archivierung erfolgreich abgeschlossen.',
        size: blob.size,
      };
